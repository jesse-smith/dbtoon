...
- [X] T007 [US4] Implement `parse_server_address` function: split on `\` for instance name, split on `,` for port, strip `tcp:` prefix, return `Result<(host, Option<u16>, Option<String>), DbtoonError>`, return `Err(DbtoonError::Config)` for invalid port values in src/backend/sqlserver.rs
