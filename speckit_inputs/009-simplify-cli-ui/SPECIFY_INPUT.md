...
The README should be rewritten to reflect the new CLI. Key changes:
- Features section: replace "read-only queries" / "write queries via exec-write" with unified `query` command + `--allow-write`; add profile management; remove env var references
- Usage section: replace all `exec-read`/`exec-write` examples with `query -P <profile>` equivalents; show `dbtoon init` as the first step; show `profile create`, `profile show`, `profile test`; show `-d`/`--database`/`--catalog` and `-s`/`--schema` overrides
- Configuration section: update config path to `~/.config/dbtoon/config.toml` (all platforms); show `dbtoon init` as the way to create it; update example TOML to match new structure with `_env` indirection
- Environment Variables section: replace the full DBTOON_* table with just the Databricks standard vars (`DATABRICKS_HOST`, `DATABRICKS_TOKEN`, `DATABRICKS_SQL_WAREHOUSE_ID`, `DATABRICKS_CATALOG`, `DATABRICKS_SCHEMA`) and note they are lowest-priority fallbacks

## Updated CLI help text

The `--help` output should reflect the new command structure. Sample output (refine during planning):

```
Multi-database query CLI with TOON output

Usage: dbtoon [OPTIONS] <COMMAND>

Commands:
  init       Create a new config file with example profiles
  query      Execute a SQL query
  profile    Manage connection profiles [create, edit, show, list, test, delete, rename]
  warehouse  Databricks warehouse commands [list]
  update     Update dbtoon to the latest release
  help       Print this message or the help of the given subcommand(s)

Options:
  -c, --config <PATH>  Path to config file [default: ~/.config/dbtoon/config.toml]
  -v, --verbose        Emit diagnostics to stderr
      --show-secrets   Disable credential masking
  -h, --help           Print help
```

`dbtoon query --help` target:
```
Execute a SQL query

Usage: dbtoon query [OPTIONS] -P <PROFILE> [SQL]

Arguments:
  [SQL]  SQL query text

Query:
  -l, --limit <N>        Max rows to return
      --no-limit         Disable row limit
  -t, --timeout <N>      Query timeout in seconds
      --allow-write      Skip read-only safety validation

Context:
  -d, --database <DB>    Override database (SQL Server) or catalog (Databricks)
      --catalog <CAT>    Alias for --database
  -s, --schema <SCHEMA>  Override schema

Output:
  -o, --output <PATH>    Write results to file (format from extension)

Connection:
  -P, --profile <NAME>   Config profile to use (required)
  -f, --file <PATH>      Read SQL from file (conflicts with positional SQL)
